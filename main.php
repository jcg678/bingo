	...
	if (isset($_POST['comprobar'])) {
		...
		for ($i = 1; $i <= 3; $i++) {
			...
			for ($j = 1; $j <= 9; $j++) {
				...
				if ($_POST[$i . "_" . $j] != 0) {
					if (in_array($_POST[$i . "_" . $j], $jugados)) {
						$repetidos++;
					}
					if (in_array($_POST[$i . "_" . $j], $fuera)) {
						$contadorAcertados++;
					}
					if (in_array($_POST[$i . "_" . $j], $fuera)) {
						$contadorLinea++;
...
//generamos la tabla con la funcion casilla solo para los casos de números que hayan salido anteriormente
	...
	if ($totalJugados != 15 and isset($_SESSION['jugados'])) {
		echo "<h3>Te faltan o te sobran números</h3>";
	}
	if ($repetidos != 0 and isset($_SESSION['jugados'])) {
		echo "<h3>Has repetido números revisa tu cartón</h3>";
	}
	if ($contadorAcertados == 15) {
		echo "<h1>BINGOOOO HAS GANADO</h1>";
	}
	if ($contadorAcertados != 15 and $comprobarLinea == 1) {
		echo "<h1>LINEAAAA</h1>";
	}
